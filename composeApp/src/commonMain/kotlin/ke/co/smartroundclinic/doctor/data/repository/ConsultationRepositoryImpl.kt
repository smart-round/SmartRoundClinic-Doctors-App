...
class ConsultationRepositoryImpl(private val client: HttpClient) : ConsultationRepository {
        ...
        withContext(Dispatchers.IO) {
            try {
                val response = client.post("consultation") {
                    setBody(StartConsultationReq(appointmentId))
                }.body<ConsultationSessionResponse>()
                if (response.status) {
                    Resource.Success(response.data?.toDomain() ?: return@withContext Resource.Error("No session data"), response.message)
    ...
    override suspend fun getMessages(sessionId: String, page: Int, size: Int): Resource<List<ConsultationMessage>> =
        withContext(Dispatchers.IO) {
            try {
                ...
                if (response.status) {
                    Resource.Success(response.data?.items?.map(ConsultationMessageData::toDomain) ?: emptyList(), response.message)
                } else Resource.Error(response.message)
            } catch (e: Exception) {
    ...
    override suspend fun endConsultation(sessionId: String): Resource<Unit> =
        withContext(Dispatchers.IO) {
            try {
                client.patch("consultation/$sessionId/end")
