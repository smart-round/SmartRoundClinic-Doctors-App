...
import ke.co.smartroundclinic.doctor.data.remote.dto.response.PresignUploadReq
import ke.co.smartroundclinic.doctor.data.remote.dto.response.PresignUploadResponse
import ke.co.smartroundclinic.doctor.data.remote.dto.response.CompleteUploadReq
...
import ke.co.smartroundclinic.doctor.data.remote.dto.request.CallActionReq
import ke.co.smartroundclinic.doctor.data.remote.dto.request.InviteToCallReq
import ke.co.smartroundclinic.doctor.data.remote.dto.response.CallActionRes
import ke.co.smartroundclinic.doctor.data.remote.dto.response.CallInviteRes
import ke.co.smartroundclinic.doctor.data.remote.dto.response.ConsultationFileUploadResponse
import ke.co.smartroundclinic.doctor.data.remote.dto.response.ConsultationMessageData
import ke.co.smartroundclinic.doctor.data.remote.dto.response.ConversationThreadMessagesResponse
import ke.co.smartroundclinic.doctor.data.remote.dto.response.ConversationThreadsResponse
...
class ConsultationRepositoryImpl(
    ...
    override suspend fun endCall(otherUserId: String): Resource<Unit> = withContext(Dispatchers.IO) {
        try {
            client.post("chat/$otherUserId/call/end")
    ...
    override suspend fun uploadFile(
        ...
        try {
            // 1. Ask the API where to put it. It enforces its own size ceiling here, so an
            //    oversized file is rejected before a single byte of it is sent.
            ...
            // 2. Stream the bytes straight to storage. Uses the auth-free client — a Bearer
            //    token on a pre-signed URL is a signature mismatch — and the Content-Type must
