...
class GetReceivedReferralsUseCase(private val repository: ReferralRepository) {
    suspend operator fun invoke(): Resource<List<Referral>> =
        when (val result = repository.getReceivedReferrals()) {
            is Resource.Success -> Resource.Success(result.data?.map { it.toDomain() } ?: emptyList())
            is Resource.Error -> Resource.Error(result.message ?: "Failed to load received referrals")
