...
import ke.co.smartroundclinic.doctor.core.notification.IncomingCallState
 ...
 * ringtone on loop, and vibrates until answered/declined/cancelled. Patient calling the doctor
 * is the only ring direction in this app (the doctor app never rings itself), so the caller is
 * always the patient.
 */
class IncomingCallActivity : ComponentActivity() {
    ...
    private fun acceptCall(callId: String, callerId: String, callerName: String?) {
        NotificationDeepLink.signal(NotificationEvent.ToCall(patientId = callerId, patientName = callerName ?: "Patient", appointmentId = "", callId = callId))
